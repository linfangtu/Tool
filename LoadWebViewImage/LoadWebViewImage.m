...
- (void)loadImageWithHtmlContent:(NSString *)htmlContont webView:(UIWebView *)webView baseUrl:(NSURL *)url {
    ...
    NSString *docPath = [path stringByAppendingPathComponent:FILE_WEBIMAGES_CACHE];
...
//                NSLog(@"正确解析出来的SRC为：%@", src);
    ...
    NSURL *pathURL = [NSURL URLWithString:docPath];
    [webView loadHTMLString:htmlContont baseURL:pathURL];
...
- (void)downloadImageWithUrl:(NSString *)src {
    ...
    [imgView setImageWithURLRequest:request placeholderImage:nil success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
        ...
        NSString *localPath = [path stringByAppendingPathComponent:FILE_WEBIMAGES_CACHE];
        BOOL isExists = [[NSFileManager defaultManager] fileExistsAtPath:localPath];
        if (!isExists) {
            [[NSFileManager defaultManager] createDirectoryAtPath:localPath withIntermediateDirectories:YES attributes:nil error:nil];
        }
        NSString *filePath = [localPath stringByAppendingPathComponent:[self md5:src]];
